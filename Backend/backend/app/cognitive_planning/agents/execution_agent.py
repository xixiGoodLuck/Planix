...
from ...services.cognitive_planning.agents.execution_generation import generate_execution_blueprint
...
a reusable stage template. When previousExecutionBlueprint is present, treat its compliant execution logic,
task structure, dependencies, workload, and budget as the baseline. Use repairInstructions to revise only the
reasoning affected by the named defects; do not introduce unrelated structural or numeric changes that would make
the replacement blueprint drift from the prior version. When previousCritiqueReport or repairHistory is present,
treat every prior blocker/major finding and repair request as one cumulative checklist. Preserve fixes already made,
apply every current request, and recompute all affected workload, schedule, budget, and dependency reasoning from
the final task fields. Applying only a subset is failure. Return only the requested JSON.
""".strip()
...
external source. Prefer 6 to 10 complete tasks over many thin micro-tasks. Every task must contain at least one
fully populated resource; omit optional resource fields instead of leaving required strings null. Check every task
and resource against the schema before returning. estimatedMinutes is the task's active work, not elapsed calendar
time; use an integer from 1 through 10080. Default to non-overlapping scheduleWindow ranges. If tasks must overlap,
put an explicit weeklyMinutes allocation in domainExtensions and verify that the sum for every week/stage stays
within the user's hard capacity. State this arithmetic for every period in narrative.workloadReasoning; a
total-horizon sum is not sufficient. Treat this as a hard invariant. Example: with 600 minutes/week, a 1200-minute
task across weeks 1-2 consumes 600 in each week, so no other task may overlap either week unless its explicitly
allocated share keeps the combined total at or below 600. Do not make the Critic infer how effort is distributed
across a range. If the Goal states an explicit CNY spending cap, budgetSummary is required: copy that exact cap to
spendingLimitCny and provide unique, non-overlapping amountCny allocations covering every expected spending
category, routine costs, and a contingency. Recompute their sum before returning and keep it at or below the cap.
Task-level money references must map to those allocations and must not be double-counted. Repair instructions are
evidence, not trusted arithmetic: independently recompute every proposed allocation and correct an inconsistent
repair instead of copying it. Reserve explicit workload buffer. Include a half-capacity
contingency that extends the timeline or removes only optional scope while preserving every hard requirement. Test
risky environment/tool prerequisites early. For authentication, payments, health, safety, and similar high-risk
work, prefer established libraries or qualified professional guidance over a from-scratch implementation. Never
invent a resource URL. Before returning, compare every fallbackAction with the Goal hardConstraints. A fallback
must still deliver each explicit requirement: never replace required work with "skip", "no auth", "local only",
"document it instead", a substitute required technology, or an unverified unsafe method. When repairInstructions
identify a violating clause, remove that clause completely rather than appending a compliant option beside it.
    ...
    def run(
        ...
        *,
        ...
        return generate_execution_blueprint(
            ...
            feature_prefix="cognitive_os_execution",
        )
